/**
 * Bonding Curve Service (Client-Side)
 * Handles buy/sell operations on bonding curves (like pump.fun)
 */
...
export async function buyFromCurve(params: BuySellParams): Promise<BuySellResult> {
  ...
  // Get M token address from curve
  const mTokenAddress = await readContract(wagmiConfig, {
  ...
  // Check and approve M tokens if needed
  ...
  // Execute buy
  console.log(`Buying ${amount} tokens for ~${quote.mCost} M`)
...
export async function getTokenBalance(
  ...
  try {
    ...
    const balance = await readContract(wagmiConfig, {
      ...
      args: [userAddress],
      chainId: 43522, // MemeCore Insectarium Testnet
    })
